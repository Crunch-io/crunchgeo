...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "crunchgeo", "tests")); system.time(test_check("crunchgeo", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-export _R_CHECK_CRAN_INCOMING_REMOTE_=FALSE && R CMD check --as-cran crunchgeo_$(VERSION).tar.gz
	rm -rf crunchgeo.Rcheck/
